...
SELECT COUNT(*) FROM cars.`car dekho` WHERE YEAR=2020;#74
SELECT COUNT(*) FROM cars.`car dekho` WHERE YEAR=2021;#7
SELECT COUNT(*) FROM cars.`car dekho` WHERE YEAR=2022;#7
...
SELECT COUNT(*) FROM cars.`car dekho` WHERE YEAR BETWEEN 2010 AND 2020;
-- ALL CARS DETAILS OF 2010 COMPLEATE LIST --
SELECT*FROM cars.`car dekho` WHERE YEAR = 2010;
...
SELECT*FROM cars.`car dekho` WHERE YEAR BETWEEN 2010 AND 2020;
-- How many cars were available in 2019?--
SELECT COUNT(*) FROM cars.`car dekho` WHERE YEAR = 2019;
-- How many cars were available in 2010 and had an automatic transmission?--
SELECT COUNT(*) FROM cars.`car dekho` WHERE YEAR = 2010 AND TRANSMISSION = 'Automatic';
-- CARS ACCORDING SELLER TYPE  in 2010? --
SELECT SELLER_TYPE, COUNT(*) FROM cars.`car dekho` WHERE YEAR = 2016 GROUP BY seller_type;
-- How many cars were available in 2010 according to the seller type (Individual or Dealer)? --
SELECT SELLER_TYPE, COUNT(*) FROM cars.`car dekho` WHERE YEAR = 2010 GROUP BY SELLER_TYPE;
-- How many cars have a selling price greater than 10 lakhs in 2016?
SELECT COUNT(*) FROM cars.`car dekho` WHERE YEAR = 2016 AND SELLING_PRICE > 1000000;
-- What is the average mileage of cars in 2022? --
SELECT AVG(CAST(SUBSTRING(MILEAGE, 1, LENGTH(MILEAGE) - 4) AS DECIMAL(5,2))) FROM cars.`car dekho` WHERE YEAR = 2022;
-- How many cars are available in each owner category in 2015? --
SELECT OWNER, COUNT(*) FROM cars.`car dekho` WHERE YEAR = 2015 GROUP BY OWNER;
-- How many cars are available with more than 5 seats and priced below 5 lakhs in 2017?--
SELECT COUNT(*) FROM cars.`car dekho` WHERE YEAR = 2017 AND SEATS > 5 AND SELLING_PRICE < 500000;
-- What is the average selling price of diesel cars in 2019?--
SELECT AVG(SELLING_PRICE) FROM cars.`car dekho` WHERE YEAR = 2019 AND FUEL = 'DIESEL';
-- What is the most common fuel type in 2020?--
SELECT FUEL, COUNT(*) FROM cars.`car dekho` WHERE YEAR = 2020 GROUP BY FUEL ORDER BY COUNT(*) DESC LIMIT 1;
-- How many cars have a mileage greater than 20 km/l and are available in 2017? --
SELECT COUNT(*) FROM cars.`car dekho` WHERE YEAR = 2017 AND CAST(SUBSTRING(MILEAGE, 1, LENGTH(MILEAGE) - 4) AS DECIMAL(5,2)) > 20;
-- INSERTION --
INSERT INTO cars.`car dekho` (Name, Year, Selling_Price, Km_Driven, Fuel, Seller_Type, Transmission, Owner, Mileage, Engine, Max_Power, Torque, Seats)
